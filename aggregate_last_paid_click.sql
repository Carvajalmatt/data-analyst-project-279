...
lead_last_click AS (
    SELECT l.lead_id, l.visitor_id, l.created_at, l.amount, l.closing_reason, l.status_id,
...
non_lead_visitors AS (
    SELECT ps.visitor_id, ps.visit_date, ps.source AS utm_source, ps.medium AS utm_medium, ps.campaign AS utm_campaign,
...
-- Unificamos todo tal cual como sale en last_paid_click.sql
last_paid_click AS (
    SELECT
    ...
    FROM non_lead_visitors WHERE rn = 1
...
-- Aquí está la magia: Usamos COUNT(DISTINCT visitor_id) para quitar a los repetidos
traffic_agg AS (
    SELECT
        DATE(visit_date) AS visit_date,
        utm_source, utm_medium, utm_campaign,
        COUNT(DISTINCT visitor_id) AS visitors_count,
        COUNT(DISTINCT visitor_id) FILTER (WHERE lead_id IS NOT NULL) AS leads_count,
